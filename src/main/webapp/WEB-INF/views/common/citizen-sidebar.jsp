    ...
    <nav style="flex:1; overflow-y:auto; padding:12px 10px;">
        <p style="padding:4px 12px 8px; font-size:10px; font-weight:700; color:#334155; text-transform:uppercase; letter-spacing:1.5px;">Menu</p>
          ...
          <a href="<%= request.getContextPath() %>/citizen/dashboard"
              style="display:flex; align-items:center; gap:10px; padding:9px 12px; border-radius:8px; margin-bottom:2px; text-decoration:none; font-size:13px; font-weight:600; background:#15342a; color:#34d399;">
          <% } else { %>
          <a href="<%= request.getContextPath() %>/citizen/dashboard"
          ...
          <% if ("my-issues".equals(activePage) || "issue-reports".equals(activePage)) { %>
          ...
          <% if ("report-issue".equals(activePage)) { %>
        ...
        <% if ("browse-issues".equals(activePage)) { %>
        <a href="<%= request.getContextPath() %>/citizen/browse-issues"
            style="display:flex; align-items:center; gap:10px; padding:9px 12px; border-radius:8px; margin-bottom:2px; text-decoration:none; font-size:13px; font-weight:600; background:#15342a; color:#34d399;">
        <% } else { %>
        <a href="<%= request.getContextPath() %>/citizen/browse-issues"
            style="display:flex; align-items:center; gap:10px; padding:9px 12px; border-radius:8px; margin-bottom:2px; text-decoration:none; font-size:13px; font-weight:600; color:#64748b;">
        <% } %>
            <svg width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-4.35-4.35m1.85-4.65a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>
            Browse Issues
        </a>
    ...
    <div style="padding:10px; border-top:1px solid #1e293b;">
        ...
        <div style="padding:12px; margin-bottom:12px; background:#15342a; border-radius:8px; display:flex; align-items:center; gap:10px;">
            ...
            <div style="flex:1; min-width:0;">
                ...
                <p style="font-size:10px; color:#94a3b8; margin:2px 0 0; text-transform:uppercase; letter-spacing:0.5px;">Citizen</p>
            </div>
        </div>
        <% } %>
                <% if ("profile".equals(activePage)) { %>
                <a href="<%= request.getContextPath() %>/citizen/profile" style="display:flex; align-items:center; gap:10px; padding:9px 12px; border-radius:8px; margin-bottom:2px; text-decoration:none; font-size:13px; font-weight:600; background:#15342a; color:#34d399;">
                <% } else { %>
                <a href="<%= request.getContextPath() %>/citizen/profile" style="display:flex; align-items:center; gap:10px; padding:9px 12px; border-radius:8px; margin-bottom:2px; text-decoration:none; font-size:13px; font-weight:600; color:#64748b;">
                <% } %>
